...
/* External Function */
extern void Decrypt(unsigned int *SrcAddr, unsigned int *DestAddr, unsigned int Size);
extern unsigned int iget_fcs(unsigned int fcs, unsigned int data);
...
static unsigned char __attribute__((aligned(4))) gs_DeviceDescriptorFS[DEVICE_DESCRIPTOR_SIZE] =
{
	18,									//  0 desc size
	(unsigned char)(DESCRIPTORTYPE_DEVICE),					//  1 desc type (DEVICE)
	(unsigned char)(FULL_USB_VER % 0x100),					//  2 USB release
	(unsigned char)(FULL_USB_VER / 0x100),					//  3 => 1.00
	0xFF,									//  4 class
	0xFF,									//  5 subclass
	0xFF,									//  6 protocol
	(unsigned char)FULL_MAX_PKT_SIZE_EP0,					 //  7 max pack size
	(unsigned char)(VENDORID % 0x100),					//  8 vendor ID LSB
	(unsigned char)(VENDORID / 0x100),					//  9 vendor ID MSB
	(unsigned char)(PRODUCTID % 0x100),					// 10 product ID LSB    (second product)
	(unsigned char)(PRODUCTID / 0x100),					// 11 product ID MSB
...
static unsigned char __attribute__((aligned(4))) gs_DeviceDescriptorHS[DEVICE_DESCRIPTOR_SIZE] =
{
	18,									//  0 desc size
	(unsigned char)(DESCRIPTORTYPE_DEVICE),    				//  1 desc type (DEVICE)
	(unsigned char)(HIGH_USB_VER % 0x100),					//  2 USB release
	(unsigned char)(HIGH_USB_VER / 0x100),					//  3 => 1.00
	0xFF,									//  4 class
	0xFF,									//  5 subclass
	0xFF,									//  6 protocol
	(unsigned char)HIGH_MAX_PKT_SIZE_EP0,					//  7 max pack size
	(unsigned char)(VENDORID    % 0x100),					//  8 vendor ID LSB
	(unsigned char)(VENDORID    / 0x100),					//  9 vendor ID MSB
	(unsigned char)(PRODUCTID % 0x100),					// 10 product ID LSB    (second product)
	(unsigned char)(PRODUCTID / 0x100),					// 11 product ID MSB
...
static const unsigned char __attribute__((aligned(4)))
gs_ConfigDescriptorFS[CONFIG_DESCRIPTOR_SIZE] = {
	// Configuration Descriptor
	0x09,									// [ 0] desc size
	(unsigned char)(DESCRIPTORTYPE_CONFIGURATION),				// [ 1] desc type (CONFIGURATION)
	(unsigned char)(CONFIG_DESCRIPTOR_SIZE % 0x100),			// [ 2] total length of data returned LSB
	(unsigned char)(CONFIG_DESCRIPTOR_SIZE / 0x100),			// [ 3] total length of data returned MSB
	...
	// Interface Decriptor
	0x09,									// [ 0] desc size
	(unsigned char)(DESCRIPTORTYPE_INTERFACE),				// [ 1] desc type (INTERFACE)
	...
	0x07,									// [ 0] desc size
	(unsigned char)(DESCRIPTORTYPE_ENDPOINT),				// [ 1] desc type (ENDPOINT)
	BULK_IN_EP|EP_ADDR_IN,							// [ 2] endpoint address: endpoint 1, IN
	EP_ATTR_BULK,								// [ 3] endpoint attributes: Bulk
	(unsigned char)(FULL_MAX_PKT_SIZE_EP1 % 0x100),				// [ 4] max packet size LSB
	(unsigned char)(FULL_MAX_PKT_SIZE_EP1 / 0x100),				// [ 5] max packet size MSB
	...
	EP_ATTR_BULK,								// [ 3] endpoint attributes: Bulk
	(unsigned char)(FULL_MAX_PKT_SIZE_EP2 % 0x100), 			// [ 4] max packet size LSB
	(unsigned char)(FULL_MAX_PKT_SIZE_EP2 / 0x100),				// [ 5] max packet size MSB
	0x00									// [ 6] polling interval (4ms/bit=time,500ms)
};
...
gs_ConfigDescriptorHS[CONFIG_DESCRIPTOR_SIZE] = {
	...
	BULK_IN_EP|EP_ADDR_IN,							// [ 2] endpoint address: endpoint 1, IN
	EP_ATTR_BULK,								// [ 3] endpoint attributes: Bulk
	(unsigned char)(HIGH_MAX_PKT_SIZE_EP1 % 0x100),				// [ 4] max packet size LSB
	(unsigned char)(HIGH_MAX_PKT_SIZE_EP1 / 0x100),				// [ 5] max packet size MSB
	0x00,									// [ 6] polling interval (4ms/bit=time,500ms)
	...
	BULK_OUT_EP|EP_ADDR_OUT,						// [ 2] endpoint address: endpoint 2, OUT
	EP_ATTR_BULK,								// [ 3] endpoint attributes: Bulk
	(unsigned char)(HIGH_MAX_PKT_SIZE_EP2 % 0x100),				// [ 4] max packet size LSB
	(unsigned char)(HIGH_MAX_PKT_SIZE_EP2 / 0x100),				// [ 5] max packet size MSB
	0x00									 // [ 6] polling interval (4ms/bit=time,500ms)
};

static void nx_usb_write_in_fifo(unsigned int ep, unsigned char *buf, int num)
{
	/* assume all data ptr is 4 bytes aligned */
	unsigned int* dwbuf = (unsigned int*)buf;
	int i;
	for(i = 0; i < (num+3)/4; i++)
...
#endif
static void nx_usb_read_out_fifo(unsigned int ep, unsigned char *buf, int num)
{
	int i;
	unsigned int *dwbuf = (unsigned int *)buf;
...
static void nx_usb_ep0_int_hndlr(USBBOOTSTATUS *pUSBBootStatus)
{
	...
	if (pUSBBootStatus->ep0_state == EP0_STATE_INIT) {
		...
		case STANDARD_GET_DESCRIPTOR:
			...
			pUSBBootStatus->Remain_size = (unsigned int)pSetupPacket->wLength;
				switch (pSetupPacket->wValue>>8) {
					case DESCRIPTORTYPE_DEVICE:
						pUSBBootStatus->Current_ptr = (unsigned char*)pUSBBootStatus->DeviceDescriptor;
				...
				case DESCRIPTORTYPE_CONFIGURATION:
					pUSBBootStatus->Current_ptr = (unsigned char*)pUSBBootStatus->ConfigDescriptor;
					pUSBBootStatus->Current_Fifo_Size = pUSBBootStatus->ctrl_max_pktsize;
...
{
	unsigned char *bulkin_buf;
	unsigned int remain_cnt;

	dev_msg("Bulk In Function\r\n");

	bulkin_buf = (unsigned char*)pUSBBootStatus->up_ptr;
	remain_cnt = pUSBBootStatus->up_size - (unsigned int)(pUSBBootStatus->up_ptr - pUSBBootStatus->up_addr);
	...
	if (remain_cnt > pUSBBootStatus->bulkin_max_pktsize) {
		...
		/*ep1 enable, clear nak, bulk, usb active, next ep2, max pkt 64*/
		pUOReg->DCSR.DEPIR[BULK_IN_EP].DIEPCTL = 1u<<31|1<<26|2<<18|1<<15 |
...
static void header_check(USBBOOTSTATUS *pUSBBootStatus, struct nx_bootheader *ptbh)
{
	...
	if (pReg_ClkPwr->SYSRSTCONFIG & 1<<14)
		Decrypt((unsigned int *)tbi, (unsigned int *)tbi, sizeof(*tbi));

	if (tbi->signature == HEADER_ID) {					/* "NSIH" */
		pUSBBootStatus->bHeaderReceived = CTRUE;
		pUSBBootStatus->RxBuffAddr =
			(unsigned char *)((unsigned char *)(tbi->loadaddr) + sizeof(*tbi));
...
static void nx_usb_int_bulkout(USBBOOTSTATUS *pUSBBootStatus,
			       struct NX_SecondBootInfo *pTBI, unsigned int fifo_cnt_byte)
{
...
#endif
		pdwBuffer = (unsigned int *)pTBI;

		nx_usb_read_out_fifo(BULK_OUT_EP,
			(unsigned char *)&pdwBuffer[pUSBBootStatus->iRxHeaderSize / 4], fifo_cnt_byte);
	...
	/* get body */
		...
		nx_usb_read_out_fifo(BULK_OUT_EP,
				     (unsigned char *)pUSBBootStatus->RxBuffAddr,
				     fifo_cnt_byte);
#if (0)
...
static int nx_usb_set_init(USBBOOTSTATUS *pUSBBootStatus)
{
	unsigned int status = pUOReg->DCSR.DSTS;				/* System status read */
	union {
		unsigned int AID;
		unsigned short SID[2];
		unsigned char BID[4];
	} USBID;
	...
	if ((USBID.SID[0] != 0) && (USBID.SID[1] != 0)) {
		pUSBBootStatus->DeviceDescriptor[8]  = (unsigned char)(USBID.BID[2]); //  8 vendor ID LSB
		pUSBBootStatus->DeviceDescriptor[9]  = (unsigned char)(USBID.BID[3]); //  9 vendor ID MSB
		pUSBBootStatus->DeviceDescriptor[10] = (unsigned char)(USBID.BID[0]); // 10 product ID LSB    (second product)
		pUSBBootStatus->DeviceDescriptor[11] = (unsigned char)(USBID.BID[1]); // 11 product ID MSB
	}
	...
	pUOReg->DCSR.DEPOR[BULK_OUT_EP].DOEPCTL =
	    1u << 31 | 1 << 26 | 2 << 18 | 1 << 15 | pUSBBootStatus->bulkout_max_pktsize << 0;

	/*bulk in ep enable, clear nak, bulk, usb active, next ep1, max pkt */
	pUOReg->DCSR.DEPIR[BULK_IN_EP].DIEPCTL =
	    0u << 31 | 1 << 26 | 2 << 18 | 1 << 15 | pUSBBootStatus->bulkin_max_pktsize << 0;
			       ...
			       struct NX_SecondBootInfo *pTBI)
{
	...
	if ((rx_status & (0xf << 17)) == SETUP_PKT_RECEIVED) {
		...
		if ((rx_status & BULK_OUT_EP) && (fifo_cnt_byte)) {
			...
			pUOReg->GCSR.GINTMSK = INT_RESUME | INT_OUT_EP | INT_IN_EP | INT_ENUMDONE | 
...
static void usb_memcpy(void *dst, const void *src, uint32_t cnt)
{
		  ...
		  struct NX_SecondBootInfo *pTBI)
{
	...
	struct nx_bootheader *ptbh = (struct nx_bootheader *)pTBI;
	struct nx_tbbinfo *tbi = &ptbh->tbbi;
	unsigned char *tb_rsa = pUSBBootStatus->RxBuffAddr_save;
	unsigned char *next = tb_rsa + sizeof(*sb_rsa);

	/* Decrypt loader body */
	if (pReg_ClkPwr->SYSRSTCONFIG & 1<<14)
		Decrypt((unsigned int*)next, (unsigned int*)next, ptbh->tbbi.loadsize);
	next += ptbh->tbbi.loadsize;

	/* copy loader header */
	usb_memcpy(tb_rsa - sizeof(*tbi), tbi, sizeof(*tbi));

	/* copy rsapublickey part. */
	usb_memcpy(((struct asymmetrickey *)tb_rsa)->rsapublickey,
	       &sb_rsa->rsapublickey, sizeof(sb_rsa->rsapublickey));

	/* split load : header + body */
	usb_memcpy(tbi->dbi[0].usbbi.split_loadaddr - sizeof(struct nx_bootheader),
	       next, tbi->dbi[0].usbbi.split_size);
	next += tbi->dbi[0].usbbi.split_size;
	usb_memcpy(tbi->dbi[1].usbbi.split_loadaddr - sizeof(struct nx_bootheader),
	       next, tbi->dbi[1].usbbi.split_size);
}
#endif
