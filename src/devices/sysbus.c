...
static const u16 g_drex_qos[NX_DREX_QOS_NUMBER][2] = {
	//  {Count, Threshold}
...
static const u8 g_PeriBusSI[4] = {
	NX_BUS_IOPERI_SI_SLOT_MP2TS,
	NX_BUS_IOPERI_SI_SLOT_USB,
	NX_BUS_IOPERI_SI_SLOT_SDMMC,
	NX_BUS_IOPERI_SI_SLOT_GMAC
};
#endif

#if (CFG_TOP_BUS_RESET == 1)
static const u8 g_TopBusSI[4] = {
	NX_BUS_TOP_SI_SLOT_VIP_0,
	NX_BUS_TOP_SI_SLOT_VIP_1,
	NX_BUS_TOP_SI_SLOT_VIP_2,
	NX_BUS_TOP_SI_SLOT_IOPERI
};
#endif
...
void system_bus_initalize(void)
{
	...
	unsigned int num_si, num_mi;
	unsigned int addr, i_slot;
#if 0
...
retry_bus_reconfig:
	...
	printf("------------- Bottom BUS ------------\r\n");
	printf("\r\n");
	printf("PrimeCell Config Reg0(0xFC0) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFC0) );
	printf("PrimeCell Config Reg1(0xFC4) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFC4) );

	printf("PrimeCell Periph Reg0(0xFE0) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFE0) );
	printf("PrimeCell Periph Reg1(0xFE4) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFE4) );
	printf("PrimeCell Periph Reg2(0xFE8) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFE8) );
	printf("PrimeCell Periph Reg3(0xFEC) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFEC) );

	printf("PrimeCell ID	 Reg0(0xFF0) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFF0) );
	printf("PrimeCell ID	 Reg1(0xFF4) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFF4) );
	printf("PrimeCell ID	 Reg2(0xFF8) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFF8) );
	printf("PrimeCell ID	 Reg3(0xFFC) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFFC) );
#endif	  /* CFG_BOTT_BUS_MSG_ON */

	num_si = mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFC0);
	num_mi = mmio_read_32(NX_VA_BASE_REG_PL301_BOTT + 0xFC4);
	...
	if (num_mi > 1) {
		...
		for (i_slot = 0; i_slot < num_mi; i_slot++) {
			mmio_write_32(addr, (0xFF000000 | i_slot));
		val = mmio_read_32(addr);
		if (val != i_slot)
			mmio_write_32(addr, (i_slot << SLOT_NUM_POS) | (i_slot << SI_IF_NUM_POS));
		}
	...
	for (i_slot = 0; i_slot < num_si; i_slot++) {
		...
		if (val != g_BottomBusSI[i_slot])
			mmio_write_32(addr, (i_slot << SLOT_NUM_POS) | (g_BottomBusSI[i_slot] << SI_IF_NUM_POS));
	}
...
#if (CFG_BOTT_BUS_MSG_ON == 1)
	printf("MI QOS Tidemark : 0x%08x\r\n", mmio_read_32(NX_BASE_ADDR_BOTT_QOS_TRDMARK) );
	printf("MI QOS Control  : 0x%08x\r\n", mmio_read_32(NX_BASE_ADDR_BOTT_QOS_CTRL) );

	printf("SI QOS Tidemark : 0x%08x\r\n", mmio_read_32(NX_BASE_ADDR_BOTT_QOS_TRDMARK + 0x20) );
	printf("SI QOS Control  : 0x%08x\r\n", mmio_read_32(NX_BASE_ADDR_BOTT_QOS_CTRL + 0x20) );

	addr = NX_BASE_ADDR_BOTT_AR;
	if (num_mi > 1) {
		for (i_slot = 0; i_slot < num_mi; i_slot++) {
			mmio_write_32(addr, (0xFF000000 | i_slot));
			printf("MI %02d - AR      : 0x%08x\r\n", i_slot, mmio_read_32(addr) );
		}
		addr = (NX_BASE_ADDR_BOTT_AR + 0x20);
	}
	...
	printf("------------- Top BUS ------------\r\n");
	printf("\r\n");
	printf("PrimeCell Config Reg0(0xFC0) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFC0) );
	printf("PrimeCell Config Reg1(0xFC4) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFC4) );

	printf("PrimeCell Periph Reg0(0xFE0) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFE0) );
	printf("PrimeCell Periph Reg1(0xFE4) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFE4) );
	printf("PrimeCell Periph Reg2(0xFE8) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFE8) );
	printf("PrimeCell Periph Reg3(0xFEC) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFEC) );

	printf("PrimeCell ID     Reg0(0xFF0) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFF0) );
	printf("PrimeCell ID     Reg1(0xFF4) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFF4) );
	printf("PrimeCell ID     Reg2(0xFF8) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFF8) );
	printf("PrimeCell ID     Reg3(0xFFC) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFFC) );
#endif	  /* CFG_TOP_BUS_MSG_ON */

	num_si = mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFC0);
	num_mi = mmio_read_32(NX_VA_BASE_REG_PL301_TOP + 0xFC4);
	...
	for (i_slot = 0; i_slot < num_si; i_slot++) {
		...
		if (val != g_TopBusSI[i_slot])
			mmio_write_32(addr, (i_slot << SLOT_NUM_POS) | (g_TopBusSI[i_slot] << SI_IF_NUM_POS));
	}
...
#if (CFG_TOP_BUS_MSG_ON == 1)
	printf("MI QOS Tidemark : 0x%08x\r\n", mmio_read_32(NX_BASE_ADDR_TOP_QOS_TRDMARK) );
	printf("MI QOS Control  : 0x%08x\r\n", mmio_read_32(NX_BASE_ADDR_TOP_QOS_CTRL) );

	printf("SI QOS Tidemark : 0x%08x\r\n", mmio_read_32(NX_BASE_ADDR_TOP_QOS_TRDMARK + 0x20) );
	printf("SI QOS Control  : 0x%08x\r\n", mmio_read_32(NX_BASE_ADDR_TOP_QOS_CTRL + 0x20) );
	...
	printf("------------- Peri BUS -----------\r\n");
	printf("\r\n");
	printf("PrimeCell Config Reg0(0xFC0) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFC0) );
	printf("PrimeCell Config Reg1(0xFC4) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFC4) );

	printf("PrimeCell Periph Reg0(0xFE0) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFE0) );
	printf("PrimeCell Periph Reg1(0xFE4) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFE4) );
	printf("PrimeCell Periph Reg2(0xFE8) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFE8) );
	printf("PrimeCell Periph Reg3(0xFEC) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFEC) );

	printf("PrimeCell ID     Reg0(0xFF0) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFF0) );
	printf("PrimeCell ID     Reg1(0xFF4) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFF4) );
	printf("PrimeCell ID     Reg2(0xFF8) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFF8) );
	printf("PrimeCell ID     Reg3(0xFFC) : 0x%08x\r\n", mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFFC) );
#endif	  /* CFG_PERI_BUS_MSG_ON */

	num_si = mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFC0);
	num_mi = mmio_read_32(NX_VA_BASE_REG_PL301_IOPERI + 0xFC4);

	/* Set SI QoS */
#if 0
	val = mmio_read_32(NX_BASE_ADDR_IOPERI_QOS_TRDMARK + 0x20) & 0xFF;
	if (val != g_PeriQoSSI[0])
		mmio_write_32(g_PeriQoSSI[0], (NX_BASE_ADDR_IOPERI_QOS_TRDMARK + 0x20));

	val = mmio_read_32(NX_BASE_ADDR_IOPERI_QOS_CTRL + 0x20) & 0xFF;
	if (val != g_PeriQoSSI[1])
		mmio_write_32(g_PeriQoSSI[1], (NX_BASE_ADDR_IOPERI_QOS_CTRL + 0x20));
#endif
	...
	for (i_slot = 0; i_slot < num_si; i_slot++) {
		...
		if (val != g_PeriBusSI[i_slot])
			mmio_write_32(addr, (i_slot << SLOT_NUM_POS) | (g_PeriBusSI[i_slot] << SI_IF_NUM_POS );
	}
	...
	// Slave Interface
	for (i_slot = 0; i_slot < num_si; i_slot++) {
		mmio_write_32( (0xFF000000 | i_slot),	addr );
		val = mmio_read_32(addr);
	if (val != g_PeriBusSI[i_slot])
		mmio_write_32( (i_slot << SLOT_NUM_POS) | (g_PeriBusSI[i_slot] << SI_IF_NUM_POS),	addr );
	}
#endif	// #if (CFG_PERI_BUS_RESET == 1)

#if (CFG_PERI_BUS_MSG_ON == 1)
	printf("MI QOS Tidemark : 0x%08x\r\n", mmio_read_32(NX_BASE_ADDR_IOPERI_QOS_TRDMARK) );
	printf("MI QOS Control  : 0x%08x\r\n", mmio_read_32(NX_BASE_ADDR_IOPERI_QOS_CTRL) );
...
void system_qos_initialize(void)
{
    ...
    mmio_write_32((NX_VA_BASE_REG_DREX + NX_DREX_BRBRSV_CFG_OFFSET), temp);

    temp    = (
	      (NX_DREX_PORT_3_W_BRB_EN_VAL  << NX_DREX_PORT_3_W_BRB_EN_SHFT)
    ...
    mmio_write_32((NX_VA_BASE_REG_DREX + NX_DREX_BRBRSV_CTRL_OFFSET), temp);
#endif
...
#ifdef aarch32
		temp = (g_drex_qos[index][1] << 16) | g_drex_qos[index][0];
		val = mmio_read_32((unsigned int)(NX_VA_BASE_REG_DREX + NX_DREX_QOS_OFFSET + (index << 3)));
		if (val != temp)
			mmio_write_32((unsigned int)(NX_VA_BASE_REG_DREX + NX_DREX_QOS_OFFSET + (index << 3)), temp);
#endif
#ifdef aarch64
		temp = (g_drex_qos[index][1] << 16) | g_drex_qos[index][0];
		val = mmio_read_32((unsigned long long)(NX_VA_BASE_REG_DREX + NX_DREX_QOS_OFFSET + (index << 3)));
		if (val != temp)
			mmio_write_32((unsigned long long)(NX_VA_BASE_REG_DREX + NX_DREX_QOS_OFFSET + (index << 3)), temp);
#endif
	}
#endif
	...
	for (index = 0; index < NX_DREX_QOS_NUMBER; index++) {
		printf("Drex QoS %02d - : 0x%08x\r\n", index, mmio_read_32(NX_VA_BASE_REG_DREX + NX_DREX_QOS_OFFSET + (index << 3)) );
	}
#endif

#if (CFG_TIEOFF_QOS_ID_RESET == 1)
	/* TIEOFF 77 */
	temp  = mmio_read_32(NX_VA_BASE_REG_TIEOFF + (77 << 2)) & ~(0xF << NX_QOS_SHFT_USB_HW);
	temp |= (NX_QOS_ID_USB_HW << NX_QOS_SHFT_USB_HW);
	mmio_write_32((NX_VA_BASE_REG_TIEOFF + (77 << 2)), temp);

	/* TIEOFF 76 */
	temp  = mmio_read_32( NX_VA_BASE_REG_TIEOFF + (76 << 2)) & ~(0xF << NX_QOS_SHFT_USB_HA);
	temp |= (NX_QOS_ID_USB_HA << NX_QOS_SHFT_USB_HA);
	mmio_write_32((NX_VA_BASE_REG_TIEOFF + (76 << 2)), temp);
	...
	mmio_write_32((NX_VA_BASE_REG_TIEOFF + (130 << 2)), temp);
	...
	mmio_write_32((NX_VA_BASE_REG_TIEOFF + (74 << 2)), temp);

	/* TIEOFF 73 */
	temp    = mmio_read_32(NX_VA_BASE_REG_TIEOFF + (73<<2)) & 0xF00;
	...
	mmio_write_32((NX_VA_BASE_REG_TIEOFF + (73 << 2)), temp);

	/* TIEOFF 72 */
	temp    = (NX_QOS_ID_MLC0_AR	<< NX_QOS_SHFT_MLC0_AR);
	mmio_write_32((NX_VA_BASE_REG_TIEOFF + (72 << 2)), temp);
	...
	mmio_write_32((NX_VA_BASE_REG_TIEOFF + (71 << 2)), temp);
	...
	mmio_write_32((NX_VA_BASE_REG_TIEOFF + (70 << 2)), temp);

	/* TIEOFF 69 */
	temp    = mmio_read_32(NX_VA_BASE_REG_TIEOFF + (69 << 2)) & 0x1F;
	...
	mmio_write_32((NX_VA_BASE_REG_TIEOFF + (69 << 2)), temp);
#endif
}
