...
{
    unsigned int     *pAddr = (unsigned int *)&pReg_DDRPHY->PHY_CON[0];
    unsigned int     temp;
    unsigned int     i;
...
void enter_self_refresh(void)
{
	...
	mmio_clear_32(&pReg_Drex->CONCONTROL, (0x1 << 5));			// afre_en[5]. Auto Refresh Counter. Disable:0, Enable:1

	/* Step  06. Disable the Dynamic Clock */
	mmio_set_32(&pReg_Drex->MEMCONTROL, (0x1 << 0));			// clk_stop_en[0] : Dynamic Clock Control   :: 1'b0 - Always running
}
...
void exit_self_refresh(void)
{
	...
	mmio_clear_32(&pReg_Drex->MEMCONTROL, (0x1 << 0));			// clk_stop_en[0] : Dynamic Clock Control :: 1'b0  - Always running
	DMC_Delay(10);

	/* Step 02. Enable the Auto refresh counter */
	mmio_set_32(&pReg_Drex->CONCONTROL, (0x1 << 5));			// afre_en[5]. Auto Refresh Counter. Disable:0, Enable:1
	DMC_Delay(10);
...
gate_err_ret:
	/* Step 06. Set the PHY for dqs pull down mode ??? */
...
//	unsigned int Deskew[4];
	...
	if (dq_fail_status == 0) {
		/* Vaile Window Margin Left */
		dq_calibration = mmio_read_32(&pReg_DDRPHY->CAL_WR_VWML[0]);
		for(slice = 0; slice < max_slice; slice++)
			vwml[slice] = (dq_calibration >> (slice*8)) & 0xFF;

		/* Vaile Window Margin Right */
		dq_calibration = mmio_read_32(&pReg_DDRPHY->CAL_WR_VWMR[0]);
		for(slice = 0; slice < max_slice; slice++)
			vwmr[slice] = (dq_calibration >> (slice*8)) & 0xFF;

		/* Vaile Window Margin Center */
		dq_calibration = mmio_read_32(&pReg_DDRPHY->CAL_WR_VWMC[0]);
		for(slice = 0; slice < max_slice; slice++)
			vwmc[slice] = (dq_calibration >> (slice*8)) & 0xFF;
#if (MEM_CALIBRATION_BITINFO == 1)
		/* Check correction value for each slice for each bit line. */
		for(slice = 0; slice < max_slice; slice++) {
			/*  */
			for(bit_line = 0; bit_line < max_bit_line; bit_line++) {
				bit_deskew[bit_line] = (mmio_read_32(&pReg_DDRPHY->WR_DESKEW_CON[slice*3]) >> (8*0)) & 0xFF;
				bit_vwmc[bit_line] = (mmio_read_32(&pReg_DDRPHY->VWMC_STAT[slice*3]) >> (8*1)) & 0xFF;
				bit_vwml[bit_line] = (mmio_read_32(&pReg_DDRPHY->VWML_STAT[slice*3]) >> (8*2)) & 0xFF;
				bit_vwmr[bit_line] = (mmio_read_32(&pReg_DDRPHY->VWMR_STAT[slice*3]) >> (8*3)) & 0xFF;

				bit_deskew[bit_line+4] = (mmio_read_32(&pReg_DDRPHY->WR_DESKEW_CON[slice*3+1]) >> (8*0)) & 0xFF;
				bit_vwmc[bit_line+4] = (mmio_read_32(&pReg_DDRPHY->VWMC_STAT[slice*3+1]) >> (8*1)) & 0xFF;
				bit_vwml[bit_line+4] = (mmio_read_32(&pReg_DDRPHY->VWML_STAT[slice*3+1]) >> (8*2)) & 0xFF;
				bit_vwmr[bit_line+4] = (mmio_read_32(&pReg_DDRPHY->VWMR_STAT[slice*3+1]) >> (8*3)) & 0xFF;
			}
...
int ddr_write_dq_calibration(void)
{
	...
	/* Step 05. Set Write Training Mode  */
	mmio_set_32  (&pReg_DDRPHY->PHY_CON[2], (0x1 << 26));			// wr_cal_mode[26] = 1, Write Training mode
	/* Step 06. Write DQ Calibration Start */
	mmio_set_32  (&pReg_DDRPHY->PHY_CON[2], (0x1 << 27));			// wr_cal_start[27] = 1
...
static int resetgen_sequence(void)
{
	...
	} while ((mmio_read_32(&pReg_DDRPHY->SHIFTC_CON) != 0x0492) && (retry--));

	MEMMSG("PHY Version: 0x%08X\r\n", mmio_read_32(&pReg_DDRPHY->VERSION_INFO));
...
//		(0x1  <<  8) |							// [    8] ctrl_shgate. 0:Gate signal length=burst length/2+N, 1:Gate signal length=burst length/2-1
	...
	if ((pSBI->DII.TIMINGDATA >> 28) == 3)		// 6 cycles
		...
		temp |= (0x6 << 17);
#endif
	mmio_write_32(&pReg_DDRPHY->PHY_CON[0], temp);

#if 0
	mmio_set_32  ( &pReg_DDRPHY->OFFSETD_CON, (0x1 << 28) );  // upd_mode[28]=1, DREX-initiated Update Mode
	mmio_clear_32( &pReg_DDRPHY->OFFSETD_CON, (0x1 << 28) );  // upd_mode[28]=0, PHY-initiated Update Mode
#endif

	temp = mmio_read_32(&pReg_DDRPHY->LP_DDR_CON[3]) & ~0x3FFF;
	temp |= 0x105E;								// cmd_active= DDR3:0x105E, LPDDDR2 or LPDDDR3:0x000E
	mmio_write_32(&pReg_DDRPHY->LP_DDR_CON[3], temp);

	temp = mmio_read_32(&pReg_DDRPHY->LP_DDR_CON[4]) & ~0x3FFF;
	temp |= 0x107F;								// cmd_default= DDR3:0x107F, LPDDDR2 or LPDDDR3:0x000F
	mmio_write_32(&pReg_DDRPHY->LP_DDR_CON[4], temp);
	...
	mmio_write_32( &pReg_DDRPHY->DRVDS_CON[1],      // 100: 48ohm, 101: 40ohm, 110: 34ohm, 111: 30ohm
...
#else
	mmio_write_32(&pReg_DDRPHY->DRVDS_CON[0], 0x00);
	mmio_write_32(&pReg_DDRPHY->DRVDS_CON[1], 0x00);
#endif

	// Driver Strength(zq_mode_dds), zq_clk_div_en[18]=Enable
	mmio_write_32(&pReg_DDRPHY->ZQ_CON, (0x1 << 27) |			// [   27] zq_clk_en. ZQ I/O clock enable.
...
//		(0x7 << 12) |							// [14:12] zq_force-impp
	...
	mmio_set_32(&pReg_DDRPHY->ZQ_CON, (0x1 << 1));				// zq_manual_str[1]. Manual Calibration Start=1
	while ((mmio_read_32(&pReg_DDRPHY->ZQ_STATUS) & 0x1) == 0);		//- PHY0: wait for zq_done
	mmio_clear_32(&pReg_DDRPHY->ZQ_CON, (0x1 << 1));			// zq_manual_str[1]. Manual Calibration Stop : 0, Start : 1

	mmio_clear_32(&pReg_DDRPHY->ZQ_CON, (0x1 << 18));			// zq_clk_div_en[18]. Clock Dividing Enable: 1, Disable : 0

	/* Step 05. Assert "dfi_init_start" from Low to High */
	mmio_write_32(&pReg_Drex->CONCONTROL,
...
//				(0x1    <<   4) |					// nexell: 0:ca swap bit, 4 & samsung drex/phy initiated bit
	...
	mmio_set_32(&pReg_Drex->CONCONTROL, (0x1 << 28));			// dfi_init_start[28]. DFI PHY initialization start

	/* Step 06. Wait until "dfi_init_complete" is set. (DLL lock will be processed.) */
	while ((mmio_read_32(&pReg_Drex->PHYSTATUS) & (0x1 << 3)) == 0);	// dfi_init_complete[3]. wait for DFI PHY initialization complete
	mmio_clear_32(&pReg_Drex->CONCONTROL, (0x1 << 28));			// dfi_init_start[28]. DFI PHY initialization clear

	/* Step 07. Set the PHY for dqs pull down mode */
	mmio_write_32(&pReg_DDRPHY->LP_CON,
...
//			(0x0 <<   4) |						// [    4] Reserved : SBZ
...
#if (CFG_NSIH_EN == 0)
	mmio_write_32(&pReg_DrexTZ->MEMBASECONFIG[1],
	...
	mmio_write_32(&pReg_DrexTZ->MEMBASECONFIG[1], (temp << 16) |		// chip_base[26:16]. AXI Base Address. if 0x40 ==> AXI base addr of memory : 0x4000_0000, 16MB unit
...
#if (CFG_NSIH_EN == 0)
#if (_DDR_CS_NUM > 1)
	mmio_write_32(&pReg_DrexTZ->MEMCONFIG[1],
				(0x0 << 20) |					// bank lsb, LSB of Bank Bit Position in Complex Interleaved Mapping 0:8, 1: 9, 2:10, 3:11, 4:12, 5:13
				(0x0 << 19) |					// rank inter en, Rank Interleaved Address Mapping
				(0x0 << 18) |					// bit sel en, Enable Bit Selection for Randomized interleaved Address Mapping
				(0x0 << 16) |					// bit sel, Bit Selection for Randomized Interleaved Address Mapping
	...
	mmio_write_32( &pReg_Drex->PRECHCONFIG1, 0xFFFFFFFF );			//- precharge cycle
	mmio_write_32( &pReg_Drex->PWRDNCONFIG,  0xFFFF00FF );			//- low power counter
#endif
	...
	mmio_write_32(&pReg_Drex->ACTIMING0.TIMINGROW,
			(tRFC << 24) | (tRRD << 20) | (tRP << 16) | (tRCD << 12) |
			(tRC << 6) | (tRAS << 0));

	mmio_write_32(&pReg_Drex->ACTIMING0.TIMINGDATA,
			(tWTR << 28) | (tWR << 24) | (tRTP << 20) | (tPPD << 17) |
			(W2W_C2C << 14) | (R2R_C2C << 12) | (nWL << 8) |
			(tDQSCK << 4) | (nRL << 0));

	mmio_write_32(&pReg_Drex->ACTIMING0.TIMINGPOWER,
			(tFAW << 26) | (tXSR << 16) | (tXP << 8) | (tCKE << 4) |
			(tMRD << 0));
#if (_DDR_CS_NUM > 1)
	mmio_write_32(&pReg_Drex->ACTIMING1.TIMINGROW,
			(tRFC << 24) | (tRRD << 20) | (tRP << 16) | (tRCD << 12) |
			(tRC << 6) | (tRAS << 0));

	mmio_write_32(&pReg_Drex->ACTIMING1.TIMINGDATA,
	...
	mmio_write_32(&pReg_Drex->ACTIMING1.TIMINGPOWER,
			(tFAW << 26) | (tXSR << 16) | (tXP << 8) | (tCKE << 4) |
			(tMRD << 0));
#endif

//	mmio_write_32(&pReg_Drex->TIMINGPZQ,   0x00004084);			//- average periodic ZQ interval. Max:0x4084
	mmio_write_32(&pReg_Drex->TIMINGPZQ, tPZQ);				//- average periodic ZQ interval. Max:0x4084

	mmio_write_32(&pReg_Drex->WRLVL_CONFIG[0], (2 << 4));			// tWLO[7:4]
//	mmio_write_32( &pReg_Drex->WRLVL_CONFIG[0], (tWLO <<  4));		// tWLO[7:4]
	...
	mmio_write_32(&pReg_Drex->ACTIMING0.TIMINGDATA, pSBI->DII.TIMINGDATA);
	mmio_write_32(&pReg_Drex->ACTIMING0.TIMINGPOWER, pSBI->DII.TIMINGPOWER);

	if (pSBI->DII.ChipNum > 1) {
		mmio_write_32(&pReg_Drex->ACTIMING1.TIMINGROW,   pSBI->DII.TIMINGROW);
		mmio_write_32(&pReg_Drex->ACTIMING1.TIMINGDATA,  pSBI->DII.TIMINGDATA);
		mmio_write_32(&pReg_Drex->ACTIMING1.TIMINGPOWER, pSBI->DII.TIMINGPOWER);
	}

//	mmio_write_32(&pReg_Drex->TIMINGPZQ,   0x00004084);			//- average periodic ZQ interval. Max:0x4084
	mmio_write_32(&pReg_Drex->TIMINGPZQ, pSBI->DII.TIMINGPZQ);		//- average periodic ZQ interval. Max:0x4084
...
//	if (is_resume == 0) {
	...
	mmio_clear_32(&pReg_DDRPHY->MDLL_CON[0], (0x1 <  5));			// ctrl_dll_on[5]=0
	do {
...
//			(0x0 <<  4) |						// [    4] Reserved : SBZ
	...
	MEMMSG("GATE CYC    = 0x%08X\r\n", mmio_read_32(&pReg_DDRPHY->SHIFTC_CON));
	MEMMSG("GATE CODE   = 0x%08X\r\n", mmio_read_32(&pReg_DDRPHY->OFFSETC_CON[0]));

	MEMMSG("Read  DQ    = 0x%08X\r\n", mmio_read_32(&pReg_DDRPHY->OFFSETR_CON[0]));
	MEMMSG("Write DQ    = 0x%08X\r\n", mmio_read_32(&pReg_DDRPHY->OFFSETW_CON[0]));
