...
//@{
	...
	struct	NX_DPC_RegisterSet
	{
		...
		volatile U32 NTSC_SCH;           // 4h  0x10
		volatile U32 NTSC_HUE;           // 5h  0x14
		volatile U32 NTSC_SAT;           // 6h  0x18
		volatile U32 NTSC_CONT;          // 7h  0x1C
		volatile U32 NTSC_BRIGHT;        // 8h  0x20
		volatile U32 NTSC_FSC_ADJH;      // 9h  0x24
		volatile U32 NTSC_FSC_ADJL;      // Ah  0x28
		...
		volatile U32 NTSC_HSVSO;         // 24h
		volatile U32 NTSC_HSOB;          // 25h
		volatile U32 NTSC_HSOE;          // 26h
		volatile U32 NTSC_VSOB;          // 27h
		volatile U32 NTSC_VSOE;          // 28h
		volatile U32 __Reserved[(0xF8/4)-0x29];///< 0x00-0x078*2 Reserved Regions
